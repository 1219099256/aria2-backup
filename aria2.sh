...
Download_aria2_conf(){
	...
	wget --no-check-certificate -N "https://raw.githubusercontent.com/1219099256/aria2-backup/master/aria2/dht6.dat"
	[[ ! -s "dht6.dat" ]] && echo -e "${Error} Aria2 DHT6文件下载失败 !" && rm -rf "${file}" && exit 1
	wget --no-check-certificate -N "https://raw.githubusercontent.com/1219099256/aria2-backup/master/aria2/core"
	[[ ! -s "core" ]] && echo -e "${Error} Aria2 core文件下载失败 !" && rm -rf "${file}" && exit 1
	wget --no-check-certificate -N "https://raw.githubusercontent.com/1219099256/aria2-backup/master/aria2/delete.sh"
	[[ ! -s "delete" ]] && echo -e "${Error} Aria2 delete.sh文件下载失败 !" && rm -rf "${file}" && exit 1
	echo '' > aria2.session
